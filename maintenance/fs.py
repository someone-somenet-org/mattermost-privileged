...
print("* ["+("%07.6g"%round(time.time() - TS_START, 5))+"] "+str(cur.rowcount)+" referenced files in fileinfo.")
...
print("* ["+("%07.6g"%round(time.time() - TS_START, 5))+"] "+str(cur.rowcount)+" referenced team icons.")
...
print("* ["+("%07.6g"%round(time.time() - TS_START, 5))+"] "+str(cur.rowcount)+" referenced profile pics.")
...
print("* ["+("%07.6g"%round(time.time() - TS_START, 5))+"] "+str(cur.rowcount)+" possibly referenced emoji.")
db_files = db_files.union({"emoji/"+val+"/image" for sublist in cur.fetchall() for val in sublist})

cur.execute("SELECT id FROM emoji")
print("* ["+("%07.6g"%round(time.time() - TS_START, 5))+"] "+str(cur.rowcount)+" possibly referenced deleted emoji.")
db_files = db_files.union({"emoji/"+val+"/image_deleted" for sublist in cur.fetchall() for val in sublist})


cur.close()
dbconn.close()
print("\n** ["+("%07.6g"%round(time.time() - TS_START, 5))+"] Files referenced in db: "+str(len(db_files)))
...
print("** ["+("%07.6g"%round(time.time() - TS_START, 5))+"] Files on filesystem: "+str(len(fs_files)))
